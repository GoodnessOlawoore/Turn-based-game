...
const Player = function(name, point, itemClass, player, weapon, power, activePath) {
    this.name = name;
    this.point = point;
    ...
    this.setData = function() {
        ...
        $('#life-'+ this.player).text(this.point);
        $('<img src="image/wp-1.png">').appendTo(".fruit-" + this.player);
        $('.strength-' + this.player).text(this.power);
    }
    //players fight logic
    this.attack = function(whichPlayer) {
        if(playerDefend == 1) {
            whichPlayer.point -= (this.power/2);
            playerDefend = 0;

            } else {
                whichPlayer.point -= this.power;
            }
                $('#life-' + whichPlayer.player).text(whichPlayer.point);
                if(whichPlayer.point <= 0) {
                    gameOverBoard();
            }
    }
...
// create Players
let player1 = new Player('Smookey', 100, 'player1', 1, 'wp-1', 10, 'image/path-1.png');
let player2 = new Player('Panda', 100, 'player2', 2, 'wp-1', 10, 'image/path-2.png');
...
function replaceWeaponOnBoard(power){
    ...
    $(".strength-" + notActivePlayer).text(power);
}
...
function fightingArea() {
    mapContainer.hide();
    $('#smookey').css('margin-left', '300px');
    $('#panda').css('margin-right', '300px');
    $(body).css({
